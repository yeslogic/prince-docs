...
Prince also supports named spot colors that can be defined with the [`@prince-color`](css-refs.md#at-prince-color) rule. An alternate color must also be specified with the [`alternate-color`](css-props#prop-alternate-color) property, using any of the valid notations for RGB, HSL or CMYK colors. This will be used in situations where the named color is not available, such as when viewing the generated PDF file on a display. Please note that [`alternate-color`](css-props#prop-alternate-color) cannot be RGBA, HSLA or CMYKA.
...
PDF/A and PDF/X files have an output intent that defines the intended output color space of the PDF file. In Prince, the output intent color profile is selected using the [`--pdf-output-intent`](command-line.md#cl-pdf-output-intent) command-line option, or using the [`prince-pdf-output-intent`](css-props#prop-prince-pdf-output-intent) property inside a [`@prince-pdf`](css-refs.md#at-prince-pdf) at-rule. Both take a filename or URL to an ICC profile as its value.

The URL specified with the [`--pdf-output-intent`](command-line.md#cl-pdf-output-intent) command-line option or the [`prince-pdf-output-intent`](css-props#prop-prince-pdf-output-intent) property will be resolved relative to the base URL of the style sheet or document in which the rule is, and needs to point to an existing color profile file.
...
The property [`prince-pdf-color-options`](css-props#prop-prince-pdf-color-options) controls the encoding of CSS RGB values representing black and grays, i.e. RGB colors with equal values of red, green and blue.
...
The property [`prince-pdf-page-colorspace`](css-props#prop-prince-pdf-page-colorspace) controls the color space of pages in the PDF file. It affects the compositing of transparent content onto the page by selecting the color space in which compositing is performed. Prince currently defaults to the RGB color space. It may be useful to set this property to avoid converting the colors of transparent content before it is composited onto the page, possibly resulting in distorted colors.


Filters
-------

The [`filter`](css-props#prop-filter) property provides graphical effects like blurring, saturating or color shifting an element. Filters can be used alone, or combined in any way. However, the order in which filters are applied matters - applying `grayscale()` after any other filter will result in a gray result.
...
This filter applies transparency to the input. The amount value defines the proportion of the conversion. This function is similar to the [`opacity`](css-props#prop-opacity) property.
...
Images can also be included in arbitrary XML documents using the CSS [`content`](css-props#prop-content) property, which can be used to specify the image filename.
...
Images can be included in arbitrary XML documents by using the CSS [`content`](css-props#prop-content) property to specify the image filename.

The [`content`](css-props#prop-content) property can specify the image filename directly, or it can take the filename from an attribute of the element to which it has been applied.
...
```
The [`content`](css-props#prop-content) property directly specifies the filename of the image that will be used as the content of the `picture` element.
...
```
The [`content`](css-props#prop-content) property specifies that the content of the `picture` element will be an image loaded from the filename specified by the `src` attribute of the element.
...
Images can also be added to style a list with the property [`list-style-image`](css-props#prop-list-style-image) (see also [List marker style](styling.md#list-marker-style)).

#### Image Orientation

Usually images have the correct orientation when included in a document. However, it can be important to honor the orientation field in the EXIF data of JPEG and TIFF images, which can be done with the [`image-orientation`](css-props#prop-image-orientation) property.

The property applies only to content images (e.g. replaced elements and generated content), not decorative images (such as [`background-image`](css-props#prop-background-image)).

#### Image Size

CSS properties also control the size of images in print. Unless an explicit size for an image is specified by using the `width` and `height` properties, Prince will determine the intrinsic size from the image resolution (DPI), which can be overridden using the [`prince-image-resolution`](css-props#prop-prince-image-resolution) property:
...
The property [`prince-background-image-resolution`](css-props#prop-prince-background-image-resolution) does the same for background images.

The property [`object-fit`](css-props#prop-object-fit) offers another mechanism to determine how an image should be resized to fit its container. The position of the image in its container can be determined with the [`object-position`](css-props#prop-object-position) property.

The [`object-fit`](css-props#prop-object-fit) and [`object-position`](css-props#prop-object-position) properties are not specific to images, but apply to all replaced content, including content inserted as [Generated Content](gen-content.md) via the [`content`](css-props#prop-content) property.

Please note that specifying [`prince-image-resolution`](css-props#prop-prince-image-resolution) and [`prince-background-image-resolution`](css-props#prop-prince-background-image-resolution), or [`object-fit`](css-props#prop-object-fit), only affects the default DPI of images, ie. it makes them physically bigger or smaller on the page, and - it does not affect the number of pixels in the image, and thus the PDF file size will be the same.

To reduce the PDF file size, JPEG images can be recompressed at a lower quality level, or PNG images be converted to JPEG, with the [`prince-image-magic`](css-props#prop-prince-image-magic) property. See also [Image Magic](cookbook#image-magic).
