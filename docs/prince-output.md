...
Duplex printing is supported with the CSS property [`-prince-pdf-duplex`](css-props.md#prop-prince-pdf-duplex). This property may be used inside the [`@prince-pdf`](css-at-rules.mn#at-prince-pdf) at-rule to set the `Duplex` property in the `PDFViewerPreferences` dictionary. The values `duplex-flip-short-edge` and `duplex-flip-long-edge` tell about how to flip the paper, while the value `simplex` suppresses duplex printing.
