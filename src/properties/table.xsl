...
<xsl:template match="function[@class='ext dep']">
    <span class="ext"><span class="dep"><xsl:value-of select="@name"/>
    <xsl:text>(&#xa0;</xsl:text>
    <xsl:for-each select="*">
    <xsl:apply-templates select="."/>
    <xsl:if test="position()!=last()">
...
<xsl:template match="keyword[@class='ext dep']">
    <span class="ext"><span class="dep"><xsl:value-of select="."/></span></span>
</xsl:template>
